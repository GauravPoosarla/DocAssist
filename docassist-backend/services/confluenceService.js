...
async function createConfluencePageWithTicket(config, ticket){
  const auth = Buffer.from(`${config.email}:${config.apiToken}`).toString("base64");
  ...
  const llmPrompt = `
You are an assistant that adds a new Confluence documentation.

Create a new confluence documentation with usual standard and structure.
This return ONLY the newly created HTML.
...
`;
  ...
  const postUrl = `https://${config.domain}.atlassian.net/wiki/rest/api/content`;
  await axios.post(
    postUrl,
    // {
    //   type: "page",
  ...
  console.log(`New Confluence page ${pageId} with highlighted changes for ticket ${ticket.id}`);
}

module.exports = { fetchConfluencePages, updateConfluencePageWithTicket, createConfluencePageWithTicket };
